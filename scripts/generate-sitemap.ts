import { TOURNAMENTS } from "../src/data";
import { COUNTRY_PAGE_ENABLED, comparePath } from "../src/router";
import { COUNTRY_CODES, slugForCode } from "../src/countrySlug";
import { analyze } from "../src/analysis";
import { enumerateMatches } from "../src/matches";
import { pairsWithMeetings } from "../src/compare";
import { writeFileSync } from "fs";
import { resolve } from "path";

const BASE = "https://worldcuparchive.net";

// lastmod is a per-content date, NOT the build date, so the sitemap only changes
// when the underlying data does — keeping <lastmod> a trustworthy crawl signal
// instead of telling Google every page changed on every deploy. A single edition
// is immutable once played, so its pages carry an (approximate) edition date;
// pages that aggregate the whole archive carry the latest edition's date.
const editionLastmod = (year: number) => `${year}-07-20`;
const years = Object.keys(TOURNAMENTS).map(Number);
const archiveLastmod = editionLastmod(Math.max(...years));

const urls: { loc: string; priority: number; changefreq: string; lastmod: string }[] = [
  { loc: "/", priority: 1.0, changefreq: "weekly", lastmod: archiveLastmod },
  // Countries hub — high-priority index linking every nation.
  { loc: "/countries/", priority: 0.9, changefreq: "monthly", lastmod: archiveLastmod },
];

for (const year of years) {
  // Trailing slash matches the 200 URL Netlify serves (pretty_urls 301s the
  // non-slash form), so canonicals/sitemap point at the real page, not a redirect.
  urls.push({ loc: `/tournaments/${year}/`, priority: 0.9, changefreq: "monthly", lastmod: editionLastmod(year) });
  // Per-match detail pages are prerendered (scripts/prerender.ts) for every
  // played knockout match, so advertise those real static pages too.
  const t = TOURNAMENTS[year];
  for (const m of enumerateMatches(t, analyze(t))) {
    if (!m.played) continue;
    urls.push({ loc: `/tournaments/${year}/matches/${m.slug}/`, priority: 0.7, changefreq: "monthly", lastmod: editionLastmod(year) });
  }
}

// Per-nation country pages are prerendered (scripts/prerender.ts) at their
// full-name slug; advertise the trailing-slash 200 URL, consistent with the
// tournament/match entries above. These fold in every edition (incl. the latest).
if (COUNTRY_PAGE_ENABLED) {
  for (const code of COUNTRY_CODES) {
    const slug = slugForCode(code);
    if (slug) urls.push({ loc: `/countries/${slug}/`, priority: 0.8, changefreq: "monthly", lastmod: archiveLastmod });
  }
}

// Head-to-head compare pages — prerendered for every pair that has met at a
// World Cup (scripts/prerender.ts). High-intent "X vs Y" queries; they too fold
// in the latest edition.
for (const [a, b] of pairsWithMeetings()) {
  urls.push({ loc: `${comparePath(a, b)}/`, priority: 0.6, changefreq: "monthly", lastmod: archiveLastmod });
}

urls.sort((a, b) => b.priority - a.priority || a.loc.localeCompare(b.loc));

const xml = [
  '<?xml version="1.0" encoding="UTF-8"?>',
  '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">',
  ...urls.map(
    (u) =>
      `  <url>\n    <loc>${BASE}${u.loc}</loc>\n    <lastmod>${u.lastmod}</lastmod>\n    <changefreq>${u.changefreq}</changefreq>\n    <priority>${u.priority.toFixed(1)}</priority>\n  </url>`
  ),
  "</urlset>",
].join("\n");

const outPath = resolve(process.cwd(), "public", "sitemap.xml");
writeFileSync(outPath, xml + "\n");
console.log(`Sitemap: ${urls.length} URLs → ${outPath}`);
