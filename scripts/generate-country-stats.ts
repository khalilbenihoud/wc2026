...
// jfjelstul spells some rounds inconsistently across editions ("quarter-final"
// vs "quarter-finals"); normalise to the app's ROUND_NAME wording.
...
const stageLabel = (s: string) =>
  STAGE_LABEL[s] ?? s.replace(/\b\w/g, (c) => c.toUpperCase());

const STAGE_ORDER = [
  "Group stage", "Final round", "Second group stage", "Round of 16",
...
// One World Cup meeting between two teams, goals/pens oriented to the
// alphabetically-first app code (a < b) so ComparePage can read them directly.
interface Meeting { year: number; stage: string; knockout: boolean; ga: number; gb: number; aet: boolean; pens: [number, number] | null }
...
async function main() {
  ...
  // Per-match meeting log (both group and knockout), so ComparePage can show
  // every World Cup encounter — not just the top-4 rivalries. Each match has two
  // appearance rows; dedupe on match_id and orient goals to the a<b code.
  const seenMatch = new Set<string>();
  for (const a of appearances) {
    if (!isMens(a.tournament_name)) continue;
    const t = toCode(a.team_name);
    const o = toCode(a.opponent_name);
    if (!t || !o || t === o) continue;
    if (seenMatch.has(a.match_id)) continue;
    seenMatch.add(a.match_id);

    let ga = Number(a.goals_for), gb = Number(a.goals_against);
    let pa = Number(a.penalties_for), pb = Number(a.penalties_against);
    let x = t, y = o;
    if (t > o) { [x, y] = [o, t]; [ga, gb] = [gb, ga]; [pa, pb] = [pb, pa]; }

    (meetings[`${x}|${y}`] ??= []).push({
      year: yearOf(a.tournament_name),
      stage: stageLabel(a.stage_name),
      knockout: a.knockout_stage === "1",
      ga, gb,
      aet: a.extra_time === "1",
      pens: a.penalty_shootout === "1" ? [pa, pb] : null,
    });
  }
  for (const list of Object.values(meetings)) {
    list.sort((m, n) => m.year - n.year || STAGE_ORDER.indexOf(m.stage) - STAGE_ORDER.indexOf(n.stage));
  }
...
`;
  ...
  // Complete head-to-head meeting log, keyed by the "A|B" pair (A < B by code).
  const orderedMeetings: Record<string, Meeting[]> = {};
  for (const key of Object.keys(meetings).sort()) orderedMeetings[key] = meetings[key];
  const h2hHeader = `// AUTO-GENERATED by scripts/generate-country-stats.ts — DO NOT EDIT BY HAND.
...
export const WC_MEETINGS: Record<string, H2HMeeting[]> = ${JSON.stringify(orderedMeetings, null, 2)};
`;
  const h2hPath = resolve(process.cwd(), "src", "countryH2H.generated.ts");
  writeFileSync(h2hPath, h2hHeader);

  console.log(`✓ ${Object.keys(out).length} nations → ${outPath}`);
  console.log(`✓ ${Object.keys(orderedMeetings).length} head-to-head pairs → ${h2hPath}`);
  if (unmatched.size > 0) {
